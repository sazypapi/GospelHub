...
import { updateAppleMusicPlaylist } from "@/utils/actions";

function AppleMusicForm({ playlists }: { playlists: AppleMusicPlaylist[] }) {
  ...
  const updatePlaylist = (
    id: string,
    field: keyof AppleMusicPlaylist,
    value: string,
  ) => {
  ...
  return (
    <div>
      <h3 className="font-azonix text-xs mb-5 sm:text-sm text-neutral-400">
        Edit Apple Music Playlists
      </h3>
      <FormContainer action={updateAppleMusicPlaylist}>
        <input
          type="hidden"
        ...
        <div className="flex justify-end w-full mt-5">
          {imageUploaded ? (
            <SubmitButton
              text="Update Apple Music Playlists"
